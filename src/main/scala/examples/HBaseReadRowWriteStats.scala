...
object HBaseReadRowWriteStats {
  ...
  object SensorRow extends Serializable {
    def parseSensorRow(result: Result): SensorRow = {
      ...
      val p2 =
        Bytes.toDouble(result.getValue(cfDataBytes, Bytes.toBytes("disp")))
      val p3 =
        Bytes.toDouble(result.getValue(cfDataBytes, Bytes.toBytes("flo")))
      val p4 =
        Bytes.toDouble(result.getValue(cfDataBytes, Bytes.toBytes("sedPPM")))
      val p5 =
        Bytes.toDouble(result.getValue(cfDataBytes, Bytes.toBytes("psi")))
      val p6 =
        Bytes.toDouble(result.getValue(cfDataBytes, Bytes.toBytes("chlPPM")))
  ...
  object SensorStatsRow {
    def convertToPutStats(
      ...
      p.add(
        cfStatsBytes,
        Bytes.toBytes("sedPPMmax"),
        Bytes.toBytes(row.maxsedPPM)
      )
      p.add(
        cfStatsBytes,
        Bytes.toBytes("sedPPMmin"),
        Bytes.toBytes(row.minsedPPM)
      )
      p.add(
        cfStatsBytes,
        Bytes.toBytes("sedPPMavg"),
        Bytes.toBytes(row.avgsedPPM)
      )
      ...
      p.add(
        cfStatsBytes,
        Bytes.toBytes("chlPPMmax"),
        Bytes.toBytes(row.maxchlPPM)
      )
      p.add(
        cfStatsBytes,
        Bytes.toBytes("chlPPMmin"),
        Bytes.toBytes(row.minchlPPM)
      )
      p.add(
        cfStatsBytes,
        Bytes.toBytes("chlPPMavg"),
        Bytes.toBytes(row.avgchlPPM)
      )
      (new ImmutableBytesWritable, p)
    }
  }

  final val tableName = "/testbinarytable1volume/testbinarytable1"
